defmodule Accountkit.Accounts.EndUserToken do
  use Ash.Resource,
    otp_app: :accountkit,
  ...
  actions do
    ...
    read :expired do
      description "Look up all expired end-user tokens."
      filter expr(expires_at < now())
    end

    read :get_token do
      description "Look up an end-user token by JTI or token, and an optional purpose."
      get? true
    ...
    action :revoked?, :boolean do
      description "Returns true if a revocation token is found for the provided end-user token"
      argument :token, :string, sensitive?: true
      argument :jti, :string, sensitive?: true
    ...
    create :revoke_token do
      description "Revoke an end-user token. Creates a revocation token for the provided token."
      accept [:extra_data]
    ...
    create :revoke_jti do
      description "Revoke an end-user token by JTI."
      accept [:extra_data]
    ...
    create :store_token do
      description "Stores an end-user token used for the provided purpose."
    ...
    destroy :expunge_expired do
      description "Deletes expired end-user tokens."
      change filter(expr(expires_at < now()))
    end

    update :revoke_all_stored_for_subject do
      description "Revokes all stored tokens for a specific end-user subject."
      accept [:extra_data]
      argument :subject, :string, allow_nil?: false, sensitive?: true
  ...
  policies do
    bypass AshAuthentication.Checks.AshAuthenticationInteraction do
      description "AshAuthentication can interact with the end-user token resource"
      authorize_if always()
    end
  end
